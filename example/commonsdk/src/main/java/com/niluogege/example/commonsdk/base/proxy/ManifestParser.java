...
public final class ManifestParser {
            ...
            if (appInfo.metaData != null) {
                for (String key : appInfo.metaData.keySet()) {
                    if (MODULE_VALUE.equals(appInfo.metaData.get(key))) {
                        appConfigs.add(parseModule(key));
                    }
                }
            }
    ...
    private static AppConfig parseModule(String className) {
        Class<?> clazz;
        try {
        ...
        if (!(appConfig instanceof AppConfig)) {
            throw new RuntimeException("Expected instanceof ConfigModule, but found: " + appConfig);
        }
